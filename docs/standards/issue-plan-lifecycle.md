...
- **Reference issues only when the code is an accepted temporary workaround.** Specifically: the code exists today as a stop-gap that will be deleted when the issue is closed. The comment must name the issue that triggers removal, and the gating must be on capability or shape (never on `feed.id`, agency, city, or any feed-specific token).
