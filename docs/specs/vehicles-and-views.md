...
5. The prediction + reconciliation algorithms that produce a `Vehicle`,
   validated against the v1 implementation in `legacy/src/utils/`.
...
```ts
// src/lib/domain/vehicle.ts (new in Phase 4)
...
departed** ([statusUtils.ts L40-L65](../../legacy/src/utils/arrival/statusUtils.ts)).
...
[arrivalUtils.calculateMultipleArrivals](../../legacy/src/utils/arrival/arrivalUtils.ts).
...
([VehicleLayer.tsx L1-L100](../../legacy/src/components/features/maps/VehicleLayer.tsx)),
but the entry point from a station card only ever opened the map scoped to
**one** vehicle's trip
([VehicleMapContent.tsx L70-L150](../../legacy/src/components/features/maps/VehicleMapContent.tsx)).
...
ports the same algorithm to `src/lib/domain/prediction/` as pure TS
...
[speedCalculationUtils.predictVehicleSpeed L52-L92](../../legacy/src/utils/vehicle/speedCalculationUtils.ts):
...
V1 in
[positionPredictionUtils.simulateMovementAlongRoute L175+](../../legacy/src/utils/vehicle/positionPredictionUtils.ts).
...
V1 in [arrivalUtils.calculateArrival L23-L68](../../legacy/src/utils/arrival/arrivalUtils.ts):
...
- **U-turns at terminus / trip flip.** V1 confirmed-no in
  [v1 §3 gaps](../../legacy/src/utils/vehicle/positionPredictionUtils.ts).
...
```ts
// src/lib/domain/pipeline/types.ts
...
V1 algorithm in
[vehicleMatchingUtils.matchVehiclesToSchedule L188-L290](../../legacy/src/utils/schedule/vehicleMatchingUtils.ts):
...
V1 in [ghostVehicleUtils.identifyGhostTrips L62-L100](../../legacy/src/utils/schedule/ghostVehicleUtils.ts).
Algorithm:
...
([vehicleMatchingUtils L260+](../../legacy/src/utils/schedule/vehicleMatchingUtils.ts)).
...
[v1 §4 GAPS](../../legacy/src/utils/schedule/vehicleMatchingUtils.ts):
...
**The v1 hack** ([vehicleMatchingUtils.ts L188-L290](../../legacy/src/utils/schedule/vehicleMatchingUtils.ts))
...
([vehicleMatchingUtils L39-L43](../../legacy/src/utils/schedule/vehicleMatchingUtils.ts)).
