...
[`src/lib/workers/gtfs.worker.ts`](src/lib/workers/gtfs.worker.ts) reads per-stop times directly from `stop_times.txt` as published in the SQLite database. No runtime interpolation. Three consumers:
...
[`src/lib/domain/predictPosition.ts`](src/lib/domain/predictPosition.ts) exposes two predictors:
...
[`src/lib/domain/predictEta.ts`](src/lib/domain/predictEta.ts) is the *only* place live GPS is currently consumed for prediction:
...
Source: [`legacy/src/utils/vehicle/`](legacy/src/utils/vehicle), [`legacy/src/utils/arrival/`](legacy/src/utils/arrival), [`legacy/src/utils/schedule/`](legacy/src/utils/schedule).
...
[`legacy/src/utils/vehicle/speedCalculationUtils.ts`](legacy/src/utils/vehicle/speedCalculationUtils.ts):
...
1. **Use the shape, not haversine.** For each consecutive `(stop[i], stop[i+1])` pair on a trip, compute distance along the polyline using the existing `projectOnPolyline` math (port from `src/lib/domain/shapeProjection.ts`). Fallback to haversine only when the trip has no `shape_id`.
...
Three pieces, all in `src/lib/domain/`, pure TS, unit-tested:
...
- Match by `(route, direction, scheduled_start_min)` with ±10 min timing tolerance (today's v2 already does this; see `src/lib/domain/reconcile.ts`).
...
- `geo.{js,ts}` — `haversineMeters`, `projectOnPolyline`, `pointAtDistance`, `bearingAtDistance`. Already exist in `src/lib/domain/shapeProjection.ts`; this phase ports them to the shared shape.
...
**Cross-repo sharing strategy (see §6.6 for details):** vendor + mirror, low ceremony. `src/lib/domain/prediction-core/` is the source of truth; `neary-gtfs/src/pipeline/lib/prediction-core/` is a mirrored copy as plain ESM `.js`. A `scripts/check-mirror.{sh,mjs}` step diffs the two in CI so drift is impossible to merge.
...
Populate at build time (already done in P1), consume in `src/lib/workers/gtfs.worker.ts`'s `getRouteMapView`, drop the runtime `projectOnPolyline` call in `buildTripShapePlan`. Cuts a measurable chunk of per-route-load CPU. Shippable independently of later phases.
...
- **Source of truth:** `src/lib/domain/prediction-core/` (TypeScript).
...
- **Mirror:** `neary-gtfs/src/pipeline/lib/prediction-core/` (plain ESM JS).
  ...
  - Header comment in each file: `// MIRRORED from neary/src/lib/domain/prediction-core/<file>.ts. Do not edit — run scripts/sync-prediction-core.mjs instead.`
...
All of these (except the RAF rate, which is map-local) live in `DEFAULT_CONFIG` in [`lib/domain/config.ts`](src/lib/domain/config.ts), keeping one source of truth.
...
1. Polls the upstream GTFS-RT VehiclePositions feed at the same cadence as `src/` (the app) (every ~15 s).
...
2. **Runtime in `src/` (the app):** when the cascade is deciding which speed to apply to a far segment, having the *empirical* median for that exact `(route, direction, segment, hour)` is better than falling back to the per-feed TOD bucket.
